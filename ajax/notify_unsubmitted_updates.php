...
try
{
  ...
  foreach ( $user_info as $info )
  {
    ...
    if( isset($submitted[$id]) && isset($unsubmitted[$id]) )
    {
      $email         = $info['email'];
      $name          = $info['name'];
      ...
      {
        if( email_unsubmitted_update_notifications($id,$name,$email,$year) ) 
        {
          $status = 'sent';
          $reply['sentids'][] = $id;
        }
...
{
  error_log("ajax_notify_unsubmitted_updates: " . $e->getMessage());
