$(function() {
  ...
  $('#send-survey-reminders').on('click',send_survey_reminders);
  $('#notify-unsubmitted-updates').on('click',notify_unsubmitted_updates);
  $('#notify-unsubmitted-surveys').on('click',notify_unsubmitted_surveys);
...
function send_id_everyone()
{
  ...
  $.ajax( {
    ...
    .done( function(data) {
      result = parse_send_email_response(data);
      $('#send-id-everyone').parent().empty().append(result);
    } )
    .fail( function(jqXHR, textStatus, errorCode) {
      alert('Failed to send ID reminders[' + errorCode + ']: ' + textStatus);
    } );
}

function send_survey_reminders()
{
  year = $('#send-survey-reminders').data('year');

  $.ajax( {
    type: 'POST',
    url: "ajax/send_survey_reminders.php",
    data: { year: year },
  } )
    .done( function(data) {
      result = parse_send_email_response(data);
      $('#send-survey-reminders').parent().empty().append(result);
    } )
    .fail( function(jqXHR, textStatus, errorCode) {
      alert('Failed to send survey reminders[' + errorCode + ']: ' + textStatus);
    } );
}

function notify_unsubmitted_updates()
{
  year = $('#notify-unsubmitted-updates').data('year');

  $.ajax( {
    type: 'POST',
    url: "ajax/notify_unsubmitted_updates.php",
    data: { year: year },
  } )
    .done( function(data) {
      result = parse_send_email_response(data);
      $('#notify-unsubmitted-updates').parent().empty().append(result);
    } )
    .fail( function(jqXHR, textStatus, errorCode) {
      alert('Failed to send unsubmitted updates reminders[' + errorCode + ']: ' + textStatus);
    } );
}

function notify_unsubmitted_surveys()
{
  year = $('#notify-unsubmitted-surveys').data('year');

  $.ajax( {
    type: 'POST',
    url: "ajax/notify_unsubmitted_surveys.php",
    data: { year: year },
  } )
    .done( function(data) {
      result = parse_send_email_response(data);
      $('#notify-unsubmitted-surveys').parent().empty().append(result);
    } )
    .fail( function(jqXHR, textStatus, errorCode) {
      alert('Failed to send unsubmitted survey reminders [' + errorCode + ']: ' + textStatus);
    } );
}

function parse_send_email_response(data)
{
  sent    = data['sent'];
  failed  = data['failed'];
